	    ...
	    @foreach ($videos as $video)
	        <div class="multiple-link-wrapper" style="width: 250px !important; display: inline-block; vertical-align: text-top;">
			    ...
			    <div class="inner" style="height: 66px; margin-top: -4px;">
			      ...
			      <p style="font-size: 0.9em; margin-top: -9px; color: #595959"><a href="{{ route('video.intro', [$video->genre, $video->watch()->titleToUrl()]) }}">{{ str_limit($video->watch()->title, 30) }}</a> • {{ Carbon\Carbon::parse($video->uploaded_at)->diffForHumans() }}</p>
			    </div>
			</div>
	    @endforeach
	</div>
