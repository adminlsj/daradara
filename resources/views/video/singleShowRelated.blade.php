<a href="{{ route('video.watch') }}?v={{ $video->id }}" class="row no-gutter">
  ...
  <div style="width: calc(100% - 198px); min-width: 50%; max-height: 90px;" class="col-xs-6 col-sm-6 col-md-6 related-watch-title">
    <h4 style="font-weight: 600; color: white;">{{ $video->title }}</h4>
    <p style="color: #bdbdbd; overflow: hidden;text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical; font-size: 0.9em; margin-top: 4px; margin-bottom: 0px; font-weight: 400 !important;">觀看次數：{{ $video->views() }}次</p>
