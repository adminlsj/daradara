<a href="{{ route('video.watch') }}?v={{ $video->id }}{{ $source == 'playlist' ? '&list='.Request::get('list') : '' }}" class="row no-gutter">
  ...
  <div style="width: calc(100% - 198px); min-width: 50%;" class="col-xs-6 col-sm-6 col-md-6 related-watch-title">
    ...
    <p style="color: #bdbdbd; overflow: hidden;text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical; font-size: 0.9em; margin-top: 5px; margin-bottom: 0px; font-weight: 400 !important;">觀看次數：{{ $video->views() }}次</p>
