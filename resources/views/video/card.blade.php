	...
	<a style="color: inherit; text-decoration: none;" href="{{ route('video.watch') }}?v={{ $video->id }}">
	    <div style="position: relative;">
		    ...
		    <div style="background-color: #e8eaed; padding: 7px 10px; height: 73px">
			    ...
			    <div style="color: darkgray; overflow: hidden;text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical; font-size: 0.95em">{{ str_ireplace('player.', '', str_ireplace('www.', '', parse_url($video->sd, PHP_URL_HOST))) }}</div>
			</div>
		</div>
	</a>
</div>
...
<div class="hidden-sm hidden-md hidden-lg related-watch-wrap hover-opacity-all" style="background-color: #F9F9F9; margin-top: 0px; margin-left: 0px; margin-right: 0px;">
	...
	<div style="position: absolute; bottom: 7px; left: 178px;">
		<div style="color: darkgray; font-size: 0.8em; margin-left: 5px;">{{ str_ireplace('player.', '', str_ireplace('www.', '', parse_url($video->sd, PHP_URL_HOST))) }}</div>
	</div>
</div>