...
@elseif ($video->caption == 'SHOW')
	<div class="hidden-xs col-sm-3 hover-opacity-all load-more-wrapper video-card" style="margin-bottom: 9px;">
		<a style="color: inherit; text-decoration: none;" href="{{ route('video.playlist') }}?list={{ $video->watch->id }}">
		    <div style="position: relative;">
		        <img class="lazy" style="width: 100%; height: 100%;" src="{{ $video->imgur16by9() }}" data-src="{{ $video->first()->imgurL() }}" data-srcset="{{ $video->imgurL() }}" alt="{{ $video->watch->title }}">
		        <span style="right: 0px; height: calc(100% - 73px)">
			      	<div style="margin: 0;position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);">
				      	<div style="font-weight: bold">{{ $video->watch->videos->count() }}</div>
				      	<i style="font-size: 1.6em; margin-right: -5px; font-weight: bold" class="material-icons">playlist_play</i>
				    </div>
			    </span>
			    <div style="background-color: #f5f5f5; padding: 7px 10px; height: 73px">
				    <div style="font-weight: bold; overflow: hidden;text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">{{ $video->watch->title }}</div>
				    <div style="color: darkgray; overflow: hidden;text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical; font-size: 0.85em; margin-top: 3px;">{{ $video->user->name }} • {{ $video->watch->videos->count() }} 部影片</div>
				</div>
			</div>
		</a>
	</div>
	...
	<div class="hidden-sm hidden-md hidden-lg related-watch-wrap hover-opacity-all load-more-wrapper" style="background-color: #F9F9F9; margin-top: 0px; margin-left: 0px; margin-right: 0px; margin-bottom: 10px">
		<a href="{{ route('video.playlist') }}?list={{ $video->watch->id }}" class="row no-gutter">
		  <div style="padding-right: 4px; width: 175px;" class="col-xs-6 col-sm-6 col-md-6">
		    <img class="lazy" style="width: 100%; height: 100%; border-top-left-radius: 3px; border-bottom-left-radius: 3px;" src="{{ $video->imgur16by9() }}" data-src="{{ $video->imgurL() }}" data-srcset="{{ $video->imgurL() }}" alt="{{ $video->watch->title }}">
