...
<div style="padding: 0px 30px 25px 30px;">
  <div class="video-parts-wrapper" style="padding-top: 25px; margin-bottom: -4px; {{ count($video->link()) == 1 ? 'display:none;' : '' }}">
    @foreach ($video->link() as $url)
  ...
  @if ($playlist != null)
    <div style="float: right; margin-top: -35px;">
      @include('video.subscribe', ['tag' => $video->playlist()->title])
    </div>
  @endif
  ...
  <h5 style="font-weight: 400; line-height: 20px; margin-bottom: 15px">
    @foreach ($video->tags() as $tag)
        <a style="margin-right: 3px; color: #4377e8;" href="{{ route('subscribe.tag') }}?query={{ $tag }}">#{{ $tag }}</a>
    @endforeach
  </h5>
