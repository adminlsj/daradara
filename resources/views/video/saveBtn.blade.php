<form id="video-save-form" action="{{ route('video.save') }}">
  ...
  <input name="save-status" type="hidden" value="{{ $saved || $listed ? true : false }}">
  <button class="single-icon-wrapper no-button-style" method="POST">
  	<div class="single-icon no-select">
	    <i style="padding-top: 7px; font-size: 21px; color: white" class="material-icons">{{ $saved || $listed ? 'add_circle' : 'add_circle_outline'}}</i>
  	</div>
  </button>
</form>