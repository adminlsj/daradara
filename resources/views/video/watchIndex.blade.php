			...
			<div class="video-sidebar-wrapper">
				@foreach (App\Blog::$structure[$genre] as $category)
					<div style="margin-bottom: 15px;">
					  <a href="{{ route('video.watch') }}?v={{ $videos[$category['value']]->first()->id }}" class="row no-gutter">
					    <div style="padding-left: 15px; padding-right: 3px; position: relative;" class="col-xs-6 col-sm-6 col-md-6">
					      ...
					      <div style="position: absolute; right:3px; bottom: 0; height: 100%; width: 25%; background-color: rgba(0,0,0,.7); text-align: center; color: white;">
					      	<div style="margin: 0;position: absolute;top: 50%; left: 50%; transform: translate(-50%, -50%);">
						      	<div style="font-size: 1em;">{{ $videos[$category['value']]->count() }}</div>
					    ...
					    <div style="padding-top: 2px; padding-right: 15px; padding-left: 3px;" class="col-xs-6 col-sm-6 col-md-6">
					      <h4 style="color:white; margin-top:0px; margin-bottom: 0px; line-height: 19px; font-size: 1.05em;">{{ $category['title'] }}【最後更新 {{ Carbon\Carbon::parse($videos[$category['value']]->first()->created_at)->format('Y.m.d') }} 】</h4>
					      <p style="color: #e5e5e5; margin-top:1px; margin-bottom: 0px; font-size: 0.85em;">{{ $videos[$category['value']]->count() }} 部影片</p>
					    </div>
					  </a>
					</div>
				@endforeach
