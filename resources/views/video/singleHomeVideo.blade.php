<div class="hidden-xs hidden-sm col-md-6">
	...
	<div style="position: relative; height: 50px">
	    ...
	    <div>
	        ...
	        <p style="color: gray; margin-top: -7px; margin-bottom: 0px; font-size: 0.85em;">觀看次數：{{ $video->views() }}次 • {{ Carbon\Carbon::parse($video->created_at)->format('Y-m-d') }}</p>
	    </div>
	</div>​
</div>
...
<div class="visible-xs visible-sm col-xs-6 col-sm-6" style="{{ $loop->iteration % 2 == 0 ? 'padding-right: 30px; padding-left:7.5px;' : 'padding-left: 30px; padding-right:7.5px;' }}">
	...
	<div style="position: relative; height: 50px;">
	    ...
	    <div>
	        ...
	        <p style="color: #a8a8a8; margin-top: -7px; margin-bottom: 0px; font-size: 0.85em;">觀看次數：{{ $video->views() }}次 • {{ Carbon\Carbon::parse($video->created_at)->format('Y-m-d') }}</p>
	    </div>
	</div>​
</div>