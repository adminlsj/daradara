<div id="singleSearchVideo" style="padding: 7px 0px;" class="multiple-link-wrapper hover-opacity-all">
  ...
  <div class="row no-gutter inner">
    ...
    <div style="padding-top: 1px; padding-right: 12px; padding-left: 4px;" class="col-xs-6 col-sm-6 col-md-7">
      <h4 style="font-weight: bold; color: #444444; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical;">{{ $video->title }}</h4>
      <p style="font-weight: bold; color: gray;"><a class="text-ellipsis-mobile" href="{{ route('user.show', $video->user->id) }}">{{ $video->user->name }} • </a><span class="text-ellipsis-mobile">觀看次數：{{ $video->views() }}次 • {{ Carbon\Carbon::parse($video->uploaded_at)->diffForHumans() }}</p>
