<div id="singleSearchVideo" style="padding: 7px 0px;" class="multiple-link-wrapper">
  ...
  <div class="row no-gutter inner">
    ...
    <div style="padding-top: 1px; padding-right: 12px; padding-left: 4px;" class="col-xs-6 col-sm-6 col-md-7">
      ...
      <p>
        <a class="text-ellipsis-mobile" href="{{ route('user.show', $video->watch()->user()) }}">{{ $video->watch()->user()->name }} • </a><span class="text-ellipsis-mobile">觀看次數：{{ $video->views() }}次 • {{ Carbon\Carbon::parse($video->uploaded_at)->diffForHumans() }}</p></span>
