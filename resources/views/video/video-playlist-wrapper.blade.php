@if ($videos)
	<div style="margin: 10px 15px 15px 15px; box-shadow: 0 4px 8px 0 rgba(0,0,0,0.1); border-radius: 3px;">
		<div style="background-color: #e1e1e1; padding: 10px; border-top-left-radius: 3px; border-top-right-radius: 3px;">
			...
			<div><a style="color: dimgray; font-weight: bold; font-size: 0.85em" href="{{ route('user.show', [$current->user->id]) }}">{{ $current->user->name }}</a></div>
		</div>
...
@foreach ($related as $video)
    <div class="related-watch-wrap hover-opacity-all {{ $loop->iteration > 30 ? 'hidden-related-video' : '' }}" style="background-color: #F9F9F9">
