...
<div id="content-div">
  <div class="row no-gutter video-show-width">
    <div id="player-div-wrapper" class="col-md-9 single-show-player fluid-player-desktop-styles" style="background-color: #141414; position: relative; overflow-y: hidden;">
      ...
      @if ($video->outsource)
        <div style="background-color: black; background-image: url('https://vdownload.hembed.com/image/icon/loading.gif?secure=sZ6Qgx3HbAFf7be7yAIGVA==,4855472894'); background-position: center; background-repeat: no-repeat; background-size: 150px; position: relative; width: 100%; height: 0; padding-bottom: 56.25%;">
      ...
      @if ($video->comic_id)
        <a id="watch-comics-link-btn" href="{{ route('comic.showCover', ['comic' => $video->comic_id]) }}" target="_blank" class="no-select" style="background-color: crimson; border: none; border-radius: 3px; color: #d9d9d9; display: block; text-decoration: none; cursor: pointer;">
