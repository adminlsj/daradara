						...
						<div id="suggested-watch-wrapper" style="padding: 0px 15px; padding-top: 8px" class="hover-opacity-all">
							<a href="{{ route('playlist.show') }}?list={{ $video->playlist()->id }}" class="row no-gutter">
							  <div style="padding-right: 4px; position: relative;" class="col-xs-6 col-sm-6 col-md-6">
							    ...
							    <span>
							      	<div style="margin: 0;position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);">
								      	<div>{{ $video->playlist()->videos()->count() }}</div>
							  ...
							  <div style="padding-left: 4px;" class="col-xs-6 col-sm-6 col-md-6 related-watch-title">
							    <h4 style="margin-top:3px; margin-bottom: 0px; line-height: 19px; font-size: 1.05em; color:#222222;">{{ $video->playlist()->title }}</h4>
							    <p style="color: dimgray; margin-top: 3px; margin-bottom: 0px; font-size: 0.85em;">{{ $video->playlist()->user()->name }}</p>
							  </div>
							</a>
						</div>
					@endif
