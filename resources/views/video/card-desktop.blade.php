	...
	<a style="font-size: 14px; text-decoration: none;" href="{{ route('video.watch').'?v='.$video->id }}" title="{{ $video->title }}">
        <div class="preview-wrapper" style="position: relative;">
		    ...
		    @if ($video->duration != null)
			    <div class="card-duration" style="position: absolute; right: 4px; bottom: 4px; color: white; background-color: rgba(0, 0, 0, 0.75); font-size: 12px; font-weight: 500; padding: 0px 5px; border-radius: 2px; z-index: 1;">
	    ...
	    <div class="card-info-wrapper">
		    <div class="card-info-title" style="color:white; font-weight: bold; overflow: hidden;text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">{{ $video->title }}</div>
		    ...
		    <div class="card-info-metadata" style="color: dimgray; overflow: hidden;text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical; font-size: 0.85em;">觀看次數：{{ $video->views() }}次 • {{ Carbon\Carbon::parse($video->created_at)->diffForHumans() }}</div>
		</div>
	</a>
</div>