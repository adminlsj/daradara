<div class="card-mobile-panel inner">
	<div style="position: relative;">
		...
		<div style="position: absolute; right: 3px; bottom: 3px;">
			@if ($video->duration != null)
			    <div class="card-mobile-duration" style="background: rgba(0, 0, 0, 0.65); padding: 0px 3px; line-height: 17px; color: #e5e5e5; font-size: 12px;">
			    	{{ $video->duration >= 3600 ? gmdate('H:i:s', $video->duration) : gmdate('i:s', $video->duration) }}
			    </div>
		    @endif

		    <div class="card-mobile-duration" style="background: rgba(0, 0, 0, 0.65); padding: 0px 3px; line-height: 17px; margin-right: 3px; color: #e5e5e5; font-size: 12px;">
		    	{{ $video->views() }}次
		    </div>
		</div>
    </div>
	...
	<div style="padding: 0 3px;">
		<div style="text-decoration: none; color: black;">
			<div class="card-mobile-title" style="margin-top: 8px; font-weight: normal; color: #e5e5e5; padding: 0px; font-size: 12px">{{ str_replace("[".$video->user->name."] ", "", $video->title) }}</div>
