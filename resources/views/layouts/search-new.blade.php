...
<form id="hentai-form" action="{{ route('home.search') }}" method="GET">
	...
	<div id="home-rows-wrapper" class="search-rows-wrapper">
		...
		<div style="{{ $doujin ? 'margin-top: -26px;' : 'margin-top: -25px;' }}" class="search-pagination mobile-search-pagination hidden-sm hidden-md hidden-lg">{!! $results->appends(request()->query())->onEachSide(1)->links() !!}</div>
