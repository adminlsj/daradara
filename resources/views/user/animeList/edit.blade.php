<form id="updateSavelistForm" action="{{ route('user.savelist.update', ['user' => $user, 'savelist' => $savelist->id]) }}" method="POST">
    {{ csrf_field() }}
    {{ method_field('PUT') }}
                    ...
                    <div class="add-to-list-content" style="color: rgba(92,114,138); min-height: 285px;">
                        <div class="row" style="margin-top: 10px; padding-right: 45px; width: 80%;">
                            <div class="filter watching-status col-md-12">
                                <h3>標題</h3>
                                <div class="bar">
                                    <input type="text" name="title" id="title" value="{{ $savelist->title }}" required>
                                </div>
                            </div>
                            ...
                            <div class="filter episodes-progress col-md-12">
                                ...
                                <div class="bar" style="height: 89px">
                                    <textarea style="line-height: 25px; height: 89px; padding-top: 8px; padding-bottom: 8px;" id="description" name="description" rows="4">{{ $savelist->description }}</textarea>
                                </div>
                            </div>
                        </div>
                            ...
                            <h3 style="font-size: 1.3rem; color: rgba(122,133,143, .9); font-weight: 400; margin-top: 30px;">隱私</h3>
                                ...
                                <div style="margin-bottom: 1px;">
                                    <div style="float: left; width: 24px; display: inline-block;">
                                        <input style="filter: brightness(1.5);" type="checkbox" name="is_private" value="1" id="is_private-{{ $savelist->id }}" {{ $savelist->is_private ? 'checked' : '' }}>
                                    </div>
                                    <div style="display: inline-block; width: calc(100% - 24px);">
                                        <label style="font-size: 1.2rem; font-weight: 400; margin-top: -1px; margin-left: -12px; padding-left: 12px;" class="no-select" for="is_private-{{ $savelist->id }}">私人清單</label>
                                    </div>
                                </div>

                            <hr style="margin-top: 9px; margin-bottom: 10px;">
                            <div class="save-delete-btn" onclick="document.getElementById('destroySavelistForm').submit();">
                                刪除
                            </div>
                        </div>
                    </div>
