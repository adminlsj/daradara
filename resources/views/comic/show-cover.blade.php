...
<div class="comics-panel-margin comics-panel-margin-top comics-panel-padding comics-thumbnail-wrapper comic-rows-wrapper" style="position: relative;">
  @for ($i = 1; $i <= $comic->pages; $i++)
    <a href="{{ route('comic.showContent', ['comic' => $comic, 'page' => $i]) }}">
      <img class="lazy comic-rows-videos-div {{ $i > 12 ? 'hidden' : '' }}" style="border-radius: 5px; margin-bottom: 4px;" src="https://i.imgur.com/WENZTSJl.jpg" data-srcset="https://t.nhentai.net/galleries/{{ $comic->galleries_id }}/{{ $i }}t.{{ App\Nhentai::$parseExt[$comic->extensions[$i - 1]] }}">
    </a>
  @endfor
