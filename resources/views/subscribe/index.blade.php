...
<div class="main-content" style="background-color: #F5F5F5;">
	<div style="padding-top: 10px;">
		<div id="subscribes-watch-wrapper" class="row no-gutter padding-setup" style="height: 90px; overflow-x: hidden; overflow-y: hidden; position: relative;">
			@foreach ($subscribes as $subscribe)
				@if ($subscribe->type == 'watch')
					<a href="{{ route('playlist.show') }}?list={{ $subscribe->playlist()->id }}" style="text-decoration: none;">
						<div class="col-xs-1" style="width: 60px; margin: 0px; padding: 0px; text-align: center; margin-right: 15px; margin-bottom: 10px;">
							<img class="lazy" style="width: 100%; height: auto; border-radius: 50%;" src="{{ $subscribe->playlist()->imgurDefaultCircleB() }}" data-src="{{ $subscribe->playlist()->videos()->first()->imgurB() }}" data-srcset="{{ $subscribe->playlist()->videos()->first()->imgurB() }}" alt="{{ $subscribe->playlist()->title }}">
							<div class="text-ellipsis" style="width: 100%; font-size: 0.8em; padding-top: 5px; color: #595959;">{{ $subscribe->playlist()->title }}</div>
						</div>
					</a>
				@else
		...
		<div class="subscribes-tab padding-setup">
			<a href="{{ route('subscribe.index') }}?g=newest" class="{{ !Request::has('g') || Request::get('g') == 'newest' ? 'active' : '' }}" style="margin-right: 5px;">最新訂閱内容</a>
			<a href="{{ route('subscribe.index') }}?g=saved" class="{{ Request::has('g') && Request::get('g') == 'saved' ? 'active' : '' }}" style="margin-right: 5px;">儲存影片</a>
		</div>
