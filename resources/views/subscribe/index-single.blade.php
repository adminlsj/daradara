<div class="hidden-md hidden-lg" style="margin-bottom: 30px;">
	<a href="{{ route('video.show') }}?v={{ $video->id }}{{ $video->playlist() ? '&list='.$video->playlist()->id : ''}}" style="text-decoration: none;">
	...
	<div class="padding-setup" style="margin-top: 10px">
		...
		<a href="{{ route('video.show') }}?v={{ $video->id }}" style="text-decoration: none; color: black;">
			<div style="margin-left: 53px; font-size: 1.1em; line-height: 19px">{{ $video->title }}</div>
			<div style="margin-left: 53px; font-size: 0.85em; color: #595959; margin-top: 3px;">{{ $video->user()->name }} • {{ Carbon\Carbon::parse($video->uploaded_at)->diffForHumans() }}</div>
		</a>
	</div>
</div>

<div id="singleSubscribeVideo" style="padding: 7px 20px;" class="multiple-link-wrapper hidden-xs hidden-sm hover-opacity-all">
  <a href="{{ route('video.show') }}?v={{ $video->id }}{{ $video->playlist() ? '&list='.$video->playlist()->id : ''}}" class="overlay"></a>
