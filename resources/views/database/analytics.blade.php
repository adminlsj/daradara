...
<div style="margin-top: 20px; margin-left: 45px; margin-right: 45px;">
	...
	<div class="row analytics-row" style="text-align: center; border-right: 1px solid black; border-left: 1px solid black">
		<div class="col-md-2" style="width: 20%;">{{ Carbon\Carbon::parse('2020-08-01 00:00:00')->addDays($count)->toDateString() }}</div>
		<div class="col-md-2" style="width: 20%">{{ $videos['anime']->sum('views') - $totals['anime'][$count - 1] }}</div>
		<div class="col-md-2" style="width: 20%">{{ $videos['variety']->sum('views') - $totals['variety'][$count - 1] }}</div>
		<div class="col-md-2" style="width: 20%">{{ $videos['drama']->sum('views') - $totals['drama'][$count - 1] }}</div>
		<div class="col-md-2" style="width: 20%">{{ $videos['hentai']->sum('views') - $totals['hentai'][$count - 1] }}</div>
	</div>
