<div class="characters-card flex-row">
    ...
    <div class="characters-description flex-column">
        <div class="characters-name characters-name-upper flex-row" style="color: rgb(92,114,138);">
            <a href="{{ route('character.show', ['character' => $character->id, 'title' => $character->getName($chinese)]) }}">{{ $character->getName($chinese) }}</a>
            @if ($character->actors->first())
                <a class="actors" href="{{ route('staff.show', ['staff' => $character->actors->first()->id, 'title' => $character->actors->first()->getName($chinese)]) }}">{{ $character->actors->first()->getName($chinese) }}</a>
            @else
                <a href="">Unknown</a>
            @endif
        </div>
