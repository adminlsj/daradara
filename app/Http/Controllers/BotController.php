...
class BotController extends Controller
{
    ...
    public function scrapeBangumiAnimeCharacterList(Request $request)
    {
        ...
        foreach ($animes as $anime) {
            ...
            if (strpos($html, '<div id="columnInSubjectA" class="column">') !== false) {
                ...
                echo "Bangumi anime id #{$anime->id} url <a href='{$url}' target='_blank'>{$url}</a> access failed<br>";
            }
        }
    }
        ...
        foreach ($characters as $character) {
            ...
            if (strpos($html, '<h1 class="nameSingle">') !== false) {
                ...
                if (strpos($html, '<span class="tip">体重: </span>') !== false) {
                    ...
                    $weight = explode('（', $weight)[0];
                    $weight = explode('-', $weight)[0];
                    if ($weight != '?' && $weight != '？' && $weight != '不详' && $weight != '不明' && $weight != '保密' && $weight != '秘密' && strpos($weight, '吨') === false) {
    ...
    {
        $animes = Anime::where('id', '>=', $request->from)->where('id', '<=', $request->to)->where('sources', 'not like', '%"anilist"%')->where('title_ro', '!=', null)->orderBy('id', 'asc')->get();
        foreach ($animes as $anime) {
    ...
    {   
        $animes = Anime::where('title_ro', 'like', "%&#039;%")->get();
        foreach ($animes as $anime) {
            $anime->title_ro = str_replace('&#039;', "'", $anime->title_ro);
            $anime->save();
        }
